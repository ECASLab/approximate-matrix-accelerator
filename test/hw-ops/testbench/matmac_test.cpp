...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < ROWS; ++i) {
    ...
    for (int k = 0; k < ROWS; ++k) {
      ...
      #else
        hw_in_mat_c[i][k] = in_mat_c[i][k] * (1 << WL);
      #endif
    }
  }
  ...
  for (int i = 0; i < ROWS; ++i) {
    for (int j = 0; j < ROWS; ++j) {
      hw_result_f[i][j] = static_cast<float>(hw_result[i][j]) * inv_alpha * inv_alpha;
    }
  }
  ...
  for (int i = 0; i < ROWS; ++i) {
    for (int j = 0; j < ROWS; ++j) {
      hw_result_f[i][j] = static_cast<float>(hw_result[i][j]) * scale * inv_alpha * inv_alpha;
    }
  }
  ama::utils::compare_results<float, float, ROWS, COLS>(hw_result_f, sw_result, err_cnt, 0.05);
  ama::utils::sign_changes<float, float, ROWS, COLS>(hw_result_f, sw_result);
